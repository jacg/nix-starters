...
#   domains    merged over the baseline network allowlist
#
    ...
    flake-utils.lib.eachSystem [ "x86_64-linux" "aarch64-linux" ]
      (system:
        let
          ...
          # What every Claude Code session needs, whatever the project.
          # Project-specific domains (crates.io, pypi.org, ...) come in through
          # `domains`. To discover what an unfamiliar project needs, build once
          # with `unrestricted = true` and read proxy.log.
          baselineDomains = {
            "anthropic.com"             = "*";
            "claude.com"                = "*";
            "github.com"                = [ "GET" "HEAD" ];
            ...
            "channels.nixos.org"        = [ "GET" "HEAD" ];
            "cache.nixos.org"           = [ "GET" "HEAD" ];
          };
          ...
          mkClaudeSandboxed =
            ...
            , unrestricted ? false  # allow every domain; run once, read proxy.log
            }:
            sbx.mkSandbox {
